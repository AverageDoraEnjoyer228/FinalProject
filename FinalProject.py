...
with st.echo(code_location='below'):
    ...
    st.write("""
        ...
        \n
        У вас может возникнуть вопрос - нет ли здесь никакой подставы, ведь я просто мог запомнить
        все статьи с их темами. Ответ - моя модель обучалась на старых статьях, а тестировать вы можете
        на новых, которые она не видела. К тому же в arXiv статей в разы больше, чем 75k на которых
        модель обучалась, поэтому даже если вы возьмете случайную статью, шанс того, что моя
        модель видела ее на этапе обучения близок к нулю.
        \n
        Также вы можете попробовать передавать
        не научные статьи из arXiv, а какие-то другие тексты (главное на **английском**), но учтите,
        что модель призвана решать определенную задачу, поэтому если вы передадите случайный текст,
        то и вряд ли получите что-то осмысленное.
        \n
        NB: иногда Streamlit шаманит, и, возможно, придется два раза нажать кнопку Run для получения предсказания.
        \n
        NB2: даже если что-то упало, не отчаивайтесь, перезагрузите страницу и попробуйте снова :)
    """)
